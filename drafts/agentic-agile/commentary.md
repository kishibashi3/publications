...
v1 は 6 つを横並びに置いていた。書き直し案は、そのうち何が原則で何が原則から出るものかを分けている。v1 に対応する ID を持たない原則は P2 だけである。
...
次に、言明の一部が前提に移った箇所が二つある。v1 A4 の前半——同じモデルが同じ誤りを共有しうること——は、指針ではなく §2 の前提に移った。A4 の残り、すなわち生成と判定を分ける部分が D3 になっている。A1 も同じ形で、前半——実行は安価であり、人間の判断が希少資源となる——が §2 の前提に移り、D1 が引き継いだのは残りの「希少な判断を、最も価値のある判断に配分する。不要な判断をなくすことも成果である」だけである。
