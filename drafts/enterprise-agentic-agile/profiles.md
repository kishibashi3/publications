...
> 位置づけ：責務仕様案（未確定）
> 対象：Cellが引き受ける責務と、その引受・合成・改訂・解除。
> 参照：共通原則は[Core](core.md)。詳細スキーマは未確定。
...
引受Cell・判断責任者・資源・対象範囲・採用版は実際の割当として明示する。
...
Profileの正本は提供側が持ち、引受側は合意して採用した版を特定する。
EnterpriseがDomain Profileを提供する場合、Enterpriseが正本を管理する。
汎用Templateの版と、実際のCellが引き受けた責務・条件の版を区別する。

| 起点 | 正本の改訂 | 採用先の変更 |
| --- | --- | --- |
| 提供側が変更したい | 自分の規範に従って改訂を承認し、新版を公開する | 引受側へ採用を提案し、引受側が判断する |
| 引受側が変更したい | 提供側へ変更を提案し、提供側が承認して新版を公開する | 引受側が内容を確認し、採用を判断する |

承認済みの旧版を上書きせず、採用する版とその内容を固定する。新しい版を公開しただけでは既存の合意は変わらない。
採用結果は双方が追跡できる記録に残し、合意済み義務の解除も相手との合意を要する。

規範の改訂は正本の所有者、採用版の更新は採用側の規範の所有者が、適用される承認手続きに従って判断する。文書の編集権だけでは、合意した義務を削除・緩和したり権限を追加したりできない。
GitHubとCLAUDE.mdでの実装例は[適用ガイド](delivery-guide.md)を参照する。

## 5. 分離可能な責務

必要なProfileを列挙してから、同じCellで担うか、別の判断責任者が引き受ける独立Cellにするかを決める。
...
Enterprise Cellは、必要な業務責務を明らかにし、顧客・業務専門家・引受候補の判断責任者と意味と境界を具体化する。
先に組織を増やす必要はない。

例えばBilling Profileなら、請求・入金・返金の整合性、請求主体の意味、確定済み請求の扱い、Contract側との責任境界、照合Evidenceを定義する。
その責務を元のCellが担うか、別の判断責任者へ移してBilling Cellを形成するかは次に決める。
