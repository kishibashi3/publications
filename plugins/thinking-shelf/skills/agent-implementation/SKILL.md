...
1. **何を実装しようとしているか / 何が壊れているか / 実害として何が観測されているか聞く**
2. **5 層構造のどこに問題があるか特定** — Layer 1〜5 のどれが歪んでいるか
3. **該当章を Read** で実装原則を確認
4. **具体的な実装案を提示** — フレームワーク非依存な構造で記述
5. **失敗モードと対策** — 暴走、ループ、文脈漏れ、コスト爆発、データ汚染、context 溢れ
6. **実害観測との突き合わせ** — 上記で挙げた懸念のうち、ユーザーが実際に観測している harm はどれか確認する
...
1. **OK な点** — 実装で適切に機能している箇所、原則と整合している部分
2. **改善ポイント** — 各課題ごとに以下のセットで提示:
   - **課題**: 何が問題か（具体的に、5層構造のどこか明示）
   - **該当原則**: 章番号 + 出典 + 原則の要約
   - **失敗モード**: 放置すると何が起きるか、どう検知するか
   - **改善案**: 現状 → 修正後の **具体的な変更**（コード断片、構造図、設定、フレームワーク差し替えなど実装可能なレベルで）
3. **実害観測の確認** — 上記の課題のうち、ユーザーが実際に観測している harm はどれか確認する。framework 由来の理論的懸念（予防的注意）と、実際に起きている問題を明示的に区別する
4. **優先順位** — 実害観測あり → 優先高、framework 由来の理論的懸念 → 優先低

**重要**: 「指摘で終わらない」。各課題には必ず **before → after の具体的な変更案** をペアで出す。「これが弱い」だけでは framework に引きずられた批判モードになる。「これを X から Y に変えれば原則 Z に整合する」のレベルで提示する。
5. **次の検証手順** — 動作確認の優先順位
