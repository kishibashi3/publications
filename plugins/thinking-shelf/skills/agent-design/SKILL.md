...
1. **ユーザーの状況を聞く** — 何を作ろうとしているか、何が壊れているか、**実害として何が観測されているか**
...
1. **OK な点** — framework と整合している部分、設計が機能している箇所
2. **改善ポイント** — 各課題ごとに以下のセットで提示:
   - **課題**: 何が問題か（具体的に、抽象論で終わらせない）
   - **該当原則**: 章番号 + 出典 + 原則の要約
   - **失敗モード**: 放置すると何が起きるか
   - **改善案**: 現状 → 修正後の **具体的な変更**（コード断片、構造図、設定変更、ファイル分割など実装可能なレベルで）
