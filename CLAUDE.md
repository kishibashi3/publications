...
- 英語版は **ja と同じディレクトリに併置** する（`foo.ja.md` の隣に `foo.md`）。翻訳が無い状態のページを新たに公開しない（`docs/en/` に残る stub 5 件は旧構想の名残で、削除は未着手の例外）
...
├── notes/                 # 公開メモ
├── en/                    # 旧構想の残骸（stub のみ。新規の英語版はここに置かない）
...
- 日本語がデフォルト。**ja を正本とする**
- 英語版は **ja と同じディレクトリに併置** する。`docs/en/` 配下には置かない
  - ja: `<name>.ja.md` / en: `<name>.md`（例: `docs/ai/agent-design/introduction.ja.md` と `introduction.md`）
  - 英語版を持たないページは `<name>.md` / `<name>.ja.md` のどちらでもよい（連載など ja 主体のページは既に `.ja.md`。sidebar link が確定しているため `.md` へ戻さない）。`<name>.md` のページに英語版を追加するときは、日本語側を `<name>.ja.md` にリネームしてから en を `<name>.md` に置く
  - 理由: (a) 既存の前例が併置である、(b) 日英を兄弟ファイルにしておくと片翻訳の drift を検出しやすい
  - `docs/en/` に残っている stub は旧構想の名残。新規の英語版をここに追加しない
...
- 英語版は同ディレクトリ併置のため、locales 構成には戻さない。en ページは sidebar に個別に追加する
